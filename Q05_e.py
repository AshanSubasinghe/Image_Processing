...
if image1 is None or image2 is None:
    ...
    segmented_image1 = cv.morphologyEx(segmented_image1, cv.MORPH_OPEN, kernel)
    segmented_image1 = cv.morphologyEx(segmented_image1, cv.MORPH_CLOSE, kernel)
    segmented_image2 = cv.morphologyEx(segmented_image2, cv.MORPH_OPEN, kernel)
    segmented_image2 = cv.morphologyEx(segmented_image2, cv.MORPH_CLOSE, kernel)
    _, labels1, stats1, _ = cv.connectedComponentsWithStats(segmented_image1, connectivity=8)
    _, labels2, stats2, _ = cv.connectedComponentsWithStats(segmented_image2, connectivity=8)
